...
with DAG(
    ...
    coleta_dados = PythonOperator(
        task_id="coleta_dados", python_callable=captura_dados
    )

    transforma_dados = PythonOperator(
        task_id="transforma_dados", python_callable=transforma
    )
    ...
    inseri_dados_tabela_dl = PostgresOperator(
        ...
        params={"table": POSTGRES_TABLE, "path": PATH_DATA_DL},
    )
    with TaskGroup(
        group_id="check_dl", default_args={"conn_id": POSTGRES_CONN_DL}
    ) as quality_check_dl:
    ...
    inseri_dados_tabela_dw = PostgresOperator(
        ...
        params={"table": POSTGRES_TABLE, "path": PATH_DATA_DW},
    )
    with TaskGroup(
        group_id="check_dw", default_args={"conn_id": POSTGRES_CONN_DW}
    ) as quality_check_dw:
